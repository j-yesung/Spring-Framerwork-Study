...
public class UploadController {
	...
	public void uploadPro(String msg, MultipartHttpServletRequest request) { // msg(text), img(file)
		...
		try {
			...
			// 파일 저장 경로
			String path = request.getRealPath("resources/save"); // 서버 상 save 폴더 위치
			log.info("*************************** path : " + path);
	...
	public ModelAndView down(int fileNum) {
		...
		File f = null;
		if (fileNum == 1) {
			f = new File("E:\\yesung\\sky.jpg");
		} else if (fileNum == 2) {
			f = new File("E:\\yesung\\sws.jpg");
		} else {
			f = new File("E:\\yesung\\호베스.png");
		}

		/*
		 * 생성자 매개변수 String viewName : view 이름 : xml 지정한 DownloadView bean의 id값 String
		 * modelName : 파라미터명 지정 (이름) Object modelName : 데이터 (다운 시킬 파일)
		 */
