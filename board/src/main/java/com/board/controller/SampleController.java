...
/* 
 * produces : 서버에서 브라우저에 리턴해주는 데이터의 형태를 지정
 * 			  브라우저에서 요청시 지정한 accept 요청 헤더값과 일치 (서버 -> 브라우저)
...
public class SampleController {
	...
	public String getText() {
		log.info("*************** getText!! ***************");
		log.info("*************** " + MediaType.TEXT_PLAIN_VALUE); // == text/plain
		 ...
		 * 위와 같이 문자열로 직접 지정할 수도 있고, MediaType 이라는 클래스를 사용할 수도 있다.
		 * */
		return "안녕하세요~"; 
	}
	...
	// 영어와 수학 점수를 요청 파라미터로 받아서 60점 미만이면 상태코드 에러 전송.
	...
	public ResponseEntity<SampleVO> gradeCheck(Integer eng, Integer math) {
		...
		if(eng < 60 || math < 60) {
			// 상태코드 502 추가, 몸체(Body)에 데이터 추가
			result = ResponseEntity.status(HttpStatus.BAD_GATEWAY).body(vo);
		} else {
			// 상태코드 200 추가, 몸체(Body)에 데이터 추가
...
//	@GetMapping("/board/{bno}")
	...
	// @RequestBody : 요청시 데이터를 JSON으로 보내고, 여기 서버에서 Bank 자바 객체로 변환해서 받기
